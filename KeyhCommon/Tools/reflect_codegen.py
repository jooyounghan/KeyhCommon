...
def generate_inl_content(all_classes, all_enums, source_filename, output_filename):
    ...
    lines.append('//')
    lines.append('// Include this file from the matching .cpp after its reflected headers.')
...
def generate_cpp_content(all_classes, header_include_path):
    """Produce out-of-line initializeMetaObject() definitions for one header."""
    lines = [
        '// AUTO-GENERATED FILE. DO NOT EDIT MANUALLY.',
        '// Generated by KeyhCommon Reflect Code Generator',
        '// Out-of-line initializeMetaObject() definitions.',
...
def patch_cpp_with_include(filepath, include_line):
    """Put the generated .inl include after the last include in the source."""
    try:
        with open(filepath, 'r', encoding='utf-8', errors='replace', newline='') as f:
            content = f.read()
    ...
    include_indexes = [i for i, line in enumerate(lines) if include.match(line)]
    insert_at = include_indexes[-1] + 1 if include_indexes else 0
...
def _cpp_name_for_header(header_basename):
...
def _output_dir_for_header(header_path, global_output_dir):
    """Return the generated/ directory for a source header."""
    root = global_output_dir if global_output_dir else os.path.dirname(header_path)
...
def main():
    ...
    for header_path in source_headers:
        generated_dir = _output_dir_for_header(header_path, global_output_dir)
        basename = os.path.basename(header_path)
        active_output_paths.add(os.path.join(generated_dir, _inl_name_for_header(basename)))
        if classes_by_header.get(header_path):
            active_output_paths.add(os.path.join(generated_dir, _cpp_name_for_header(basename)))
    removed_includes = 0
    removed_files = 0
    ...
    for header_path in header_files:
        ...
        output_dir = _output_dir_for_header(header_path, global_output_dir)
        ...
        cpp_path = os.path.join(output_dir, _cpp_name_for_header(os.path.basename(header_path)))
        if cpp_path not in active_output_paths and remove_generated_inl(cpp_path):
            removed_files += 1
            if args.verbose:
    ...
    for header_path in source_headers:
        ...
        output_path = os.path.join(output_dir, output_name)
        cpp_name = _cpp_name_for_header(header_basename)
        cpp_path = os.path.join(output_dir, cpp_name)
